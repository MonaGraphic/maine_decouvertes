...
== Changelog ==
= 1.15 =
* Security: use one parsed attachment ID for both capability checks and crop/editor logic (fixes absint vs FILTER_SANITIZE_NUMBER_INT mismatch, e.g. attachmentId=4.1).
* Security: editor window reads postId only from POST (same source as the auth check; closes $_REQUEST vs $_GET mismatch).
* Security: enqueue crop assets/nonce only for users with upload_files.
