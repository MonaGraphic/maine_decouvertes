...
class ManualImageCropGifFix {
    ...
    public function handleGifCrop() {
        ...
        // Prefer the ID already authorized for this request.
        $attachment_id = ! empty( $GLOBALS['mic_authorized_attachment_id'] )
            ? absint( $GLOBALS['mic_authorized_attachment_id'] )
            : ( function_exists( 'mic_parse_attachment_id' ) ? mic_parse_attachment_id( $_POST['attachmentId'] ) : absint( $_POST['attachmentId'] ) );
