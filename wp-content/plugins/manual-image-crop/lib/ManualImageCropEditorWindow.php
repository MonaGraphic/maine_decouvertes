...
class ManualImageCropEditorWindow {
	...
	public function renderWindow() {
		...
		// Same source as mic_ajax_authorize_editor_window(): POST only (JS posts these fields).
		if (!isset($_POST['mainPreviewMaxWidth']) || !isset($_POST['mainPreviewMaxHeight']) || 
			!isset($_POST['smallPreviewMaxWidth']) || !isset($_POST['smallPreviewMaxHeight'])) {
			wp_die('Error: Preview size configuration parameters are missing. This could cause incorrect image cropping.');
		}
		
		$mainPreviewMaxWidth = intval($_POST['mainPreviewMaxWidth']);
		$mainPreviewMaxHeight = intval($_POST['mainPreviewMaxHeight']);
		$smallPreviewMaxWidth = intval($_POST['smallPreviewMaxWidth']);
		$smallPreviewMaxHeight = intval($_POST['smallPreviewMaxHeight']);
...
<div class="mic-editor-wrapper">
	<div class="edit-attachment-frame mode-select hide-menu hide-router">
		<div class="attachment-media-view landscape">
			...
			$editedSize = in_array( $size, $imageSizes, true ) ? $size : null;
			// Reuse the ID authorized in mic_ajax_authorize_editor_window() — never re-read a different request bucket.
			if ( empty( $GLOBALS['mic_authorized_attachment_id'] ) ) {
				wp_die( esc_html__( 'Invalid attachment.', 'microp' ), 400 );
			}
			$postId = absint( $GLOBALS['mic_authorized_attachment_id'] );
