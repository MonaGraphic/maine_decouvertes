...
/**
 * Parse an attachment ID the same way everywhere (absint only — never FILTER_SANITIZE_NUMBER_INT).
 *
 * @param mixed $value Raw request value.
 * @return int
 */
function mic_parse_attachment_id( $value ) {
	if ( is_array( $value ) ) {
		return 0;
	}
 ...
 *
 * @param int $attachment_id Attachment post ID (already parsed).
 */
 ...
 * Resolves the attachment ID once; cropImage() must reuse this same ID.
 */
function mic_ajax_authorize_crop_image() {
	$attachment_id = isset( $_POST['attachmentId'] ) ? mic_parse_attachment_id( $_POST['attachmentId'] ) : 0;
	mic_ajax_require_crop_permission( $attachment_id );
	$GLOBALS['mic_authorized_attachment_id'] = $attachment_id;
}

/**
 * Early auth for mic_editor_window (HTML response on failure).
 * Resolves postId once from POST (same source as renderWindow()).
 */
function mic_ajax_authorize_editor_window() {
	...
	$attachment_id = isset( $_POST['postId'] ) ? mic_parse_attachment_id( $_POST['postId'] ) : 0;
	if ( ! $attachment_id || 'attachment' !== get_post_type( $attachment_id ) ) {
		status_header( 400 );
